...
router.get('/:id', async (req, res) => {
    try {
        const { id } = req.params;
        const activity = await Activity_1.default.findById(id).populate('user', 'username firstName lastName');
        if (!activity) {
...
router.put('/:id', async (req, res) => {
    try {
        ...
        const activity = await Activity_1.default.findByIdAndUpdate(id, updates, { new: true });
        if (!activity) {
            return res.status(404).json({ error: 'Activity not found' });
        }
        res.json({
